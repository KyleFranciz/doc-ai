...
@app.post(
    "/api/prompt", response_model=DocsResponse
...
async def askDoc(
    ...
    try:  # try to create a table in supabase
        ...
        UserQuestion = (
            ...
            .insert(
                {  # get back the state of the UserQuestion
        ...
        if stream:
            ...
            )  # give the question that the user had and also the session_id of the user
            ...
            if not UserQuestion.data:
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to add the messages to the database"
                )
            ...
            if not SentChatSession.data:
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Failed to add chat session info to the database",
                )

            # show the data that was added if it was a success
            print(
                UserQuestion.data, DocsAnswer.data
            )  # show the data that I added to triple check        # return the response model with the filled out info to the user
            return DocsResponse(
        ...
        print("Traceback (finding the real error):", traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"There has been an error: {str(error)}"
        )
...
async def stream_doc_response(user_request: MessageRequest):
    ...
    try:
        ...
        async for token in getKnowledgeFromDocStreaming(
            user_request.question, user_request.session_id
        ):
        ...
        error_message = (
            f"data: {json.dumps({'type': 'error', 'message': str(error)})}\n\n"
        )
        yield error_message
...
async def get_msg_for_chat(
    ...
    try:
        ...
        if response.data:
            ...
            return {"session_id": session_id, "messages": [], "amount_of_messages": 0}
        else:
            raise HTTPException(
                status_code=404,
        ...
        # log the error
        raise HTTPException(
            status_code=404, detail=f"failed to fetch the message: {err}"
        )
