...
if isinstance(url, str) and isinstance(
    key, str
):  # if the url and key are correctly brought in as strings
...
def get_all_messages_4_doc(
    session_id: str,
):  # pass in the session ID from the request to place into the function
    ...
    data = (
        response.data or []
    )  # checks if I get back data, if I dont then make data an empty list
    ...
    for categories in data:
        ...
        content = categories[
            "content"
        ]  # get all the content and store it in the content var
...
def getFirstMessage(
    session_id: str,
):  # todo: add user_id as a param later on to the function
